...
figure(110)
subplot(2,1,1)
plot(v_T(v_lim_T)/12,m_x_s_traj(1:n_s_plot,v_lim_T),'b:',...
     v_T(v_lim_T)/12,v_x_tilde(            v_lim_T),'r-')
ylabel('Reser. Vol. [ x10^6m^3 ]')
xlabel('t [year]')
%legend('x_{tilde}','x')
title(['Monthly Control, Volume Tracking, with Q/R = ' num2str(ratio_q_on_r) ] ,'fontweight','normal')
grid on

subplot(2,1,2)
plot(v_T(v_lim_T)/12,m_u_s_traj(1:n_s_plot,v_lim_T),'b:',...
     v_T(v_lim_T)/12,v_u_tilde(            v_lim_T),'r-',...
     v_T(v_lim_T)/12,                  v_c(v_lim_T),'g-')
ylabel('Outflow & Inflow  [ x10^6m^3/day ]')
xlabel('t [year]')
%legend('u_{tilde}','u','c')
title('Monthly Control, Reservoir Release and Inflow Over Time','fontweight','normal')
grid on
...
figure(111)
subplot(2,1,1)
plot(v_T(v_lim_T)/12, v_x_s_lower(  :,v_lim_T),'b:',...
     v_T(v_lim_T)/12, v_x_tilde(      v_lim_T),'r--',...
     v_T(v_lim_T)/12, v_x_s_upper(  :,v_lim_T),'b:',...
     v_T(v_lim_T)/12, v_x_s_average(:,v_lim_T),'b--')
ylabel('Reser. Vol. [ x10^6m^3 ]')
xlabel('t [year]')
title(['Monthly Control, Volume Tracking, with Q/R = ' num2str(ratio_q_on_r) ] ,'fontweight','normal')
...
subplot(2,1,2)
plot(v_T(v_lim_T)/12,v_u_s_lower(  :,v_lim_T),'b:',...
     v_T(v_lim_T)/12,v_u_tilde(      v_lim_T),'r-',...
     v_T(v_lim_T)/12,            v_c(v_lim_T),'g-',...
     v_T(v_lim_T)/12,v_u_s_upper(  :,v_lim_T),'b:',...
     v_T(v_lim_T)/12,v_u_s_average(:,v_lim_T),'b--')
ylabel('Outflow & Inflow  [ x10^6m^3/day ]')
xlabel('t [year]')
